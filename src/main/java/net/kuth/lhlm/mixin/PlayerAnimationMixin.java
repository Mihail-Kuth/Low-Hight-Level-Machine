	...
	public void setupAnim(PlayerRenderState renderState, CallbackInfo ci) {
		...
		if (!master.equals("lowhight_level_machine")) {
			if (!LowhightLevelMachineModPlayerAnimationAPI.animations.isEmpty())
				LowhightLevelMachineModPlayerAnimationAPI.animations.clear();
			return;
		}
		Player player = (Player) renderState.getRenderData(LowhightLevelMachineModPlayerAnimationAPI.ClientAttachments.PLAYER);
		if (player == null)
		...
		if (data.getBooleanOr("ResetPlayerAnimation", false)) {
			...
			LowhightLevelMachineModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		LowhightLevelMachineModPlayerAnimationAPI.PlayerAnimation animation = LowhightLevelMachineModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = LowhightLevelMachineModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				LowhightLevelMachineMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			LowhightLevelMachineModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, LowhightLevelMachineModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			LowhightLevelMachineModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
